...
def addUserMONGO():
    mongo.db.penes.insert_one(request.get_json())
    return jsonify({'message':'USUARIO AÑADIDO'})

@app.route('/getUsersMONGO', methods=['GET'])
def getUsersMONGO():
    usuarios = mongo.db.penes.find()
    return jsonify({'message':usuarios})

@app.route('/editUserMONGO', methods=['PUT'])
def editUserMONGO():
    data = request.get_json()
    mongo.db.penes.update_one({"nombre":data["nombre"]},{"$set":data})
    return jsonify({'message':"USUARIO ACTUALIZADO"})

@app.route('/deleteUserMONGO', methods=['DELETE'])
def deleteUserMONGO():
    data = request.get_json()
    mongo.db.penes.delete_one({"nombre":data["nombre"]})
    return jsonify({'message':"USUARIO ELIMINADO"})
